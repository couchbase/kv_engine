...
STAT(ep_num_ops_get_meta, count, num_ops, LABEL(op, get_meta))
STAT(ep_num_ops_set_meta, count, num_ops, LABEL(op, set_meta))
STAT(ep_num_ops_del_meta, count, num_ops, LABEL(op, del_meta))
STAT(ep_num_ops_set_meta_res_fail, count, num_ops_failed, LABEL(op, set_meta))
STAT(ep_num_ops_del_meta_res_fail, count, num_ops_failed, LABEL(op, del_meta))
STAT(ep_num_ops_set_ret_meta, count, num_ops, LABEL(op, set_ret_meta))
STAT(ep_num_ops_del_ret_meta, count, num_ops, LABEL(op, del_ret_meta))
STAT(ep_num_ops_get_meta_on_set_meta, count, num_ops, LABEL(op, get_meta))
...
STAT(cmd_set, count, operations, LABEL(op, set))
...
#define VB_AGG_STAT(name, unit, familyName, ...) \
    STAT(vb_active_##name,                       \
         unit,                                   \
         familyName,                             \
         LABEL(state, active),                   \
         ##__VA_ARGS__)                          \
    STAT(vb_replica_##name,                      \
         unit,                                   \
         familyName,                             \
         LABEL(state, replica),                  \
         ##__VA_ARGS__)                          \
