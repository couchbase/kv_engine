...
TEST_F(CollectionsTest, ScopeIDLookup2) {
    auto rv = engine->get_collection_meta(*cookie, key1.getCollectionID());
    EXPECT_EQ(cb::engine_errc::success, rv.result);
    EXPECT_EQ(0, rv.getManifestId());
    EXPECT_EQ(ScopeID(ScopeID::Default), rv.getScopeId());

    rv = engine->get_collection_meta(*cookie, key2.getCollectionID());
    EXPECT_EQ(cb::engine_errc::success, rv.result);
    EXPECT_EQ(0, rv.getManifestId());
    EXPECT_EQ(ScopeID(ScopeID::Default), rv.getScopeId());

    rv = engine->get_collection_meta(*cookie, key3.getCollectionID());
