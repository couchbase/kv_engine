...
void Settings::reconfigure(const nlohmann::json& json) {
    ...
    for (const auto& obj : json.items()) {
        ...
        if (key == "always_collect_trace_info"sv) {
            ...
            setDcpDisconnectWhenStuckTimeout(std::chrono::seconds(value.get<size_t>()));
...
void Settings::updateSettings(const Settings& other, bool apply) {
    ...
    if (other.has.max_so_rcvbuf_size) {
        if (other.max_so_rcvbuf_size != max_so_rcvbuf_size) {
            LOG_INFO("Change max SO_RCVBUF from {} to {}",
                     max_so_rcvbuf_size,
                     other.max_so_rcvbuf_size);
    ...
    if (other.has.not_locked_returns_tmpfail) {
        if (other.getNotLockedReturnsTmpfail() !=
            ...
            LOG_INFO("Change not_locked_returns_tmpfail from: {} to {}",
                     getNotLockedReturnsTmpfail(),
                     other.getNotLockedReturnsTmpfail());
