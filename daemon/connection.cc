...
void Connection::copyToOutputStream(gsl::span<std::string_view> data) {
    ...
    std::array<evbuffer_iovec, 2> iov;
    ...
    for (auto d : data) {
        ...
        if (nwritten < iov[0].iov_len) {
            // fill data into iov[0]
            auto nbytes = std::min(iov[0].iov_len - nwritten, d.size());
            auto* dest = static_cast<char*>(iov[0].iov_base) + nwritten;
            std::copy(d.data(), d.data() + nbytes, dest);
            nwritten += nbytes;
            d = {d.data() + nbytes, d.size() - nbytes};
        }

        if (!d.empty()) {
            // fill data into iov[1] and there SHOULD be room!
            const auto nbytes = d.size();
            const auto curroffset = nwritten - iov[0].iov_len;
            if (iov[1].iov_len + curroffset < nbytes) {
                throw std::runtime_error(
                        "copyToOutputStream(): Invalid buffer returned");
            }

            auto* dest = static_cast<char*>(iov[1].iov_base) + curroffset;
            std::copy(d.data(), d.data() + nbytes, dest);
            nwritten += nbytes;
        }
    }

    // reserve may have returned more memory than I requested so we need
    // to adujst the sizes in the iovector (otherwise we'll end up send
    // more data than intended)
    if (nentries == 1) {
        iov[0].iov_len = total_size;
    } else {
        iov[1].iov_len = total_size - iov[0].iov_len;
    }

    if (evbuffer_commit_space(out, iov.data(), nentries) == -1) {
        throw std::bad_alloc();
    }

    totalSend += total_size;
    get_thread_stats(this)->bytes_written += total_size;
}
