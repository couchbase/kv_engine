...
static nlohmann::json create_memcached_audit_object(
        const Connection& c,
        const boost::optional<cb::mcbp::request::EffectiveUser>& euid) {
    nlohmann::json root;
    ...
    if (euid) {
        root["effective_userid"]["domain"] = to_string(euid->domain);
        root["effective_userid"]["user"] = euid->username;
    }
...
void audit_command_access_failed(const Cookie& cookie) {
    ...
    auto root = create_memcached_audit_object(
            connection, cookie.getRequest().getEffectiveUser());
    char buffer[256];
