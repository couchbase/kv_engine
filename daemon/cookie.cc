...
void Cookie::logSpuriousNotification(cb::engine_errc status,
    ...
    const auto cookiePtr = fmt::format("{}", static_cast<const void*>(this));

    // Log what should be stack stored data first and flush the logger. I.e. as
    // it stands we don't trust this.
    LOG_CRITICAL_CTX(
            "Spurious notification: cookie not in ewouldblock state [1/2] "
            "notification state",
            {"cookie", cookiePtr},
            {"magic", cb::to_hex(notification_magic_byte)},
            {"generation", notification_generation},
            {"opcode", opToString(notification_opcode)},
    ...
    // don't know.
    LOG_CRITICAL_CTX(
            "Spurious notification: cookie not in ewouldblock state [2/2] "
            "cookie state",
            {"cookie", cookiePtr},
            {"magic",
             cb::to_hex(magic_byte_pattern.load(std::memory_order_acquire))},
