...
std::unique_ptr<AuditEventFilter> AuditConfig::createAuditEventFilter(
        uint64_t rev) {
    std::vector<std::pair<std::string, cb::rbac::Domain>> u;
    {
        std::lock_guard<std::mutex> guard(disabled_userids_mutex);
        for (const auto& pair : disabled_userids) {
            if (pair.first == "local") {
                u.emplace_back(std::pair<std::string, cb::rbac::Domain>{
                        pair.second, cb::rbac::Domain::Local});
            } else {
                u.emplace_back(std::pair<std::string, cb::rbac::Domain>{
                        pair.second, cb::rbac::Domain::External});
            }
        }
    }
