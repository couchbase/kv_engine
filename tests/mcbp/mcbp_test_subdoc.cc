...
TEST_F(SubdocMultiLookupTest, ValidLocationOpcodes) {
  ...
  request.clearLookups();
  request.addLookup({PROTOCOL_BINARY_CMD_SUBDOC_GET, SUBDOC_FLAG_NONE, "[0]"});
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_SUCCESS, validate(request));
}
...
protected:
  ...
  void testFlags(protocol_binary_subdoc_flag pathFlag,
                 ...
                 protocol_binary_response_status expected, uint8_t spec) {
...
TEST_F(SubdocMultiMutationTest, ValidDictAdd) {
  ...
  request.addMutation({PROTOCOL_BINARY_CMD_SUBDOC_DICT_ADD,
                       protocol_binary_subdoc_flag(0), "path", "value"});
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_SUCCESS, validate(request));
  testFlagCombo(SUBDOC_FLAG_MKDIR_P, mcbp::subdoc::doc_flag::Mkdoc,
                PROTOCOL_BINARY_RESPONSE_SUCCESS, 1);
}

TEST_F(SubdocMultiMutationTest, InvalidDictAdd) {
  request.addMutation({PROTOCOL_BINARY_CMD_SUBDOC_DICT_ADD,
                       protocol_binary_subdoc_flag(0xff), "path", "value"});
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_EINVAL, validate(request));
  ...
  // Must have value.
  request.at(1) = {PROTOCOL_BINARY_CMD_SUBDOC_DICT_ADD,
                   protocol_binary_subdoc_flag(0), "path", ""};
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_EINVAL, validate(request));
}
...
TEST_F(SubdocMultiMutationTest, InvalidDelete) {
  ...
  testFlags(SUBDOC_FLAG_MKDIR_P, mcbp::subdoc::doc_flag::Mkdoc,
            PROTOCOL_BINARY_RESPONSE_EINVAL, 1);
...
TEST_F(SubdocMultiMutationTest, ValidReplace) {
  request.addMutation({PROTOCOL_BINARY_CMD_SUBDOC_REPLACE,
                       protocol_binary_subdoc_flag(0), "path", "new_value"});
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_SUCCESS, validate(request));
}
...
TEST_F(SubdocMultiMutationTest, InvalidReplace) {
  // Must have path.
  request.addMutation({PROTOCOL_BINARY_CMD_SUBDOC_REPLACE,
                       protocol_binary_subdoc_flag(0), "", "new_value"});
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_EINVAL, validate(request));
  ...
  // Shouldn't have flags.
  request.at(1) = {PROTOCOL_BINARY_CMD_SUBDOC_REPLACE, SUBDOC_FLAG_NONE, "path",
                   "new_value"};
  testFlags(SUBDOC_FLAG_MKDIR_P, mcbp::subdoc::doc_flag::Mkdoc,
            PROTOCOL_BINARY_RESPONSE_EINVAL, 1);
}
...
TEST_F(SubdocMultiMutationTest, InvalidArrayPushLast) {
  // Only allowed empty flags or SUBDOC_FLAG_MKDIR_P (0x1)
  request.addMutation({PROTOCOL_BINARY_CMD_SUBDOC_ARRAY_PUSH_LAST,
                       protocol_binary_subdoc_flag(0xff), "", "value"});
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_EINVAL, validate(request));
...
TEST_F(SubdocMultiMutationTest, InvalidArrayInsert) {
  ...
  request.at(1) = {PROTOCOL_BINARY_CMD_SUBDOC_ARRAY_INSERT, SUBDOC_FLAG_NONE,
                   "path", "value"};
  testFlags(SUBDOC_FLAG_MKDIR_P, mcbp::subdoc::doc_flag::Mkdoc,
            PROTOCOL_BINARY_RESPONSE_EINVAL, 1);
  ...
  request.at(1) = {PROTOCOL_BINARY_CMD_SUBDOC_ARRAY_INSERT,
                   protocol_binary_subdoc_flag(0), "", "value"};
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_EINVAL, validate(request));
...
TEST_F(SubdocMultiMutationTest, WholeDocDeleteInvalidPath) {
  ...
  // Must not have path.
  request.addMutation({PROTOCOL_BINARY_CMD_DELETE,
                       protocol_binary_subdoc_flag(0), "_sync", ""});
  EXPECT_EQ(PROTOCOL_BINARY_RESPONSE_EINVAL, validate(request));
}
...
TEST_F(SubdocMultiMutationTest, ValidWholeDocDeleteFlags) {
  request.clearMutations();
  request.addMutation(
      {PROTOCOL_BINARY_CMD_DELETE, protocol_binary_subdoc_flag(0), "", ""});
