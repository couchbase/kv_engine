...
TEST_P(RegressionTest, MB_31070) {
    ...
    auto metadata = conn.getMeta(name, 0, GetMetaVersion::V1);
    EXPECT_LT(0, metadata.second.expiry);

    document.info.expiration = 0;
    conn.mutate(document, 0, MutationType::Append);

    auto metadata2 = conn.getMeta(name, 0, GetMetaVersion::V1);
    EXPECT_NE(0, metadata2.second.expiry);
}
