...
extern "C" {
    ...
    static ENGINE_ERROR_CODE syncCmd(EventuallyPersistentEngine *e,
        ...
        void *data = e->getEngineSpecific(cookie);
        ...
        if (!validFlags) {
            return sendResponse(response, NULL, 0, NULL, 0, "", 0,
                                PROTOCOL_BINARY_RAW_BYTES,
...
bool EventuallyPersistentEngine::parseConfig(const char *config,
                                             struct config_item *items) {
    EventuallyPersistentEngine *epe = ObjectRegistry::onSwitchThread(NULL, true);
    bool rv = serverApi->core->parse_config(config, items, stderr) == 0;
...
static void add_casted_stat(const char *k, const char *v,
    ...
    // Set the thread local to NULL as it is going to call add_stat memcached callback.
    ...
    // Set the thread local to its old value again.
...
EventuallyPersistentEngine::handleGetLastClosedCheckpointId(const void *cookie,
    ...
    return sendResponse(response, NULL, 0, NULL, 0, &checkpointId, sizeof(checkpointId),
                        PROTOCOL_BINARY_RAW_BYTES,
                        PROTOCOL_BINARY_RESPONSE_SUCCESS, 0, cookie);
}
