...
size_t ExecutorPool::calcNumNonIO(size_t threadCount) const {
    ...
    // DurabilityCompletionTask) and hence we have increased the coefficient,
    // based on empirical testing. We could likely increase further, however if
    // there was a problem with other NonIO tasks consuming excessive CPU that
    // could start to impact front-end threads, so let's increase this
    // gradually...
