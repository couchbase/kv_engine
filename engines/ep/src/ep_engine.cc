...
cb::engine_errc EventuallyPersistentEngine::cache_transfer_rx(
        CookieIface& cookie,
        uint32_t opaque,
        Vbid vbucket,
    ...
    auto engine = acquireEngine(this);
    auto conn = engine->getConnHandler(cookie);
    return conn->cache_transfer_rx(opaque, vbucket, items);
}
...
void EventuallyPersistentEngine::doEngineStatsFusion(
    ...
    constexpr std::array<std::string_view, 32> statNames = {
            {"fusion_NumSyncs",
             ...
             "fusion_PendingUploadBytes",
             "fusion_PendingCheckpointBytes",
    ...
    addStat(Key::ep_fusion_pending_checkpoint_bytes,
            "fusion_PendingCheckpointBytes");
    ...
    addStat(Key::ep_fusion_checkpoint_failures, "fusion_NumCheckpointFailures");
...
cb::EngineErrorItemPair EventuallyPersistentEngine::getRandomDocument(
    ...
    if (es.has_value()) {
        ...
        if (cb::mcbp::datatype::is_xattr(es.value()->item->getDataType())) {
            ...
            if (checkCollectionAccess(
                ...
                es.value()->item->removeSystemXattrs();
            }
        }
...
void EventuallyPersistentEngine::handleDisconnect(CookieIface& cookie) {
    ...
    kvBucket->releaseBlockedCookie(cookie);
}
