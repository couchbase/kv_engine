...
struct DCPBackfillIface {
     ...
     * - e.g. run() threw an exception (which the caller has logged). The
     * backfill will not be run again and will be destroyed after this call.
     *
     * Implementations should propagate the failure to any associated stream.
     * Must not throw.
     */
    virtual void fail() = 0;
