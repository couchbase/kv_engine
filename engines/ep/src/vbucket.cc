...
VBNotifyCtx VBucket::queueItem(queued_item& item, const VBQueueItemCtx& ctx) {
    ...
    std::unique_lock<std::mutex> durLock(dmQueueMutex);
    ...
    // Notify the DM of a non-SyncWrite so that we can move the HPS if possible
    if (!item->isPending()) {
        durabilityMonitor->notifyNonPrepare(*item);
    }
