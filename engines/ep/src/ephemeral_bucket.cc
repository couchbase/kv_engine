...
bool EphemeralBucket::canPageItems(size_t limit) {
    maxBytesDeleteableByPager = limit;
    // Continue paging if we are not yet at the limit.
    if (maxBytesDeleteableByPager > bytesPagerDeleted) {
        EP_LOG_DEBUG(
                "EphemeralBucket::canPageItems: Continue paging with limit "
                "{} bytes, maxBytesDeleteableByPager: {} bytes, "
                "bytesPagerDeleted: {} bytes.",
                limit,
                maxBytesDeleteableByPager,
                bytesPagerDeleted);
        return true;
    }

    // We must be above the limit. Can only resume if the vbucket's confirm that
    // they have drained away deletions generated by paging, the assumption
    // being that if true then memory has truly reduced and more paging can
    // occur.
    // There could still be tombstones in the linked list that are not yet
    // purged - those can only be removed when the metadata purge interval
    // elapses.
    for (auto vbid : getVBucketsInState(vbucket_state_active)) {
        auto vb = getVBucket(vbid);
        if (!vb) {
            continue;
        }
        auto& evb = static_cast<EphemeralVBucket&>(*vb);
        if (!evb.canPagingResume()) {
            EP_LOG_DEBUG(
                    "EphemeralBucket::canPageItems: Pager throttled by "
                    "vbucket {}, limit: "
                    "{} bytes, bytesPagerDeleted: {} bytes.",
                    vbid,
                    maxBytesDeleteableByPager,
                    bytesPagerDeleted);
            ++pagerThrottled;
    ...
    EP_LOG_DEBUG(
            "EphemeralBucket::canPageItems: Pager can run, all VBS "
            "satisfied. limit: {} bytes, bytesPagerDeleted: {} bytes.",
            maxBytesDeleteableByPager,
            bytesPagerDeleted);
    // We were at the limit and now all vbuckets confirm enough deletes have
    // drained away, paging can resume.
    bytesPagerDeleted = 0;
    return true;
}

bool EphemeralBucket::shouldContinuePaging(size_t bytes) {
    const auto previous = bytesPagerDeleted.fetch_add(bytes);
    const auto current = previous + bytes;
    EP_LOG_DEBUG(
            "EphemeralBucket::shouldContinuePaging: Paged out {} bytes, "
            "current is "
            "{} bytes, max is {} bytes",
            bytes,
            current,
            maxBytesDeleteableByPager);
    return maxBytesDeleteableByPager == 0 ||
           maxBytesDeleteableByPager > current;
}

bool EphemeralBucket::shouldContinuePaging() const {
    return maxBytesDeleteableByPager == 0 ||
           maxBytesDeleteableByPager > bytesPagerDeleted;
}

void EphemeralBucket::addStats(const BucketStatCollector& collector) {
    collector.addStat(cb::stats::Key::pager_deleted, bytesPagerDeleted);
    collector.addStat(cb::stats::Key::pager_throttled, pagerThrottled);
}
