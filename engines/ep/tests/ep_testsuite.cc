...
static enum test_result test_access_scanner(EngineIface* h) {
    ...
    if (access_scanner_skips == 0) {
        // Access scanner has not ran so trigger and wait until equal num_shards
        checkeq(cb::engine_errc::success,
                set_param(h,
                          EngineParamCategory::Flush,
                          "access_scanner_run",
                          "true"),
                "Failed to trigger access scanner");
        wait_for_stat_to_be(h, "ep_num_access_scanner_skips", num_shards);
    } else {
        // Access scanner ran so expect num_shards
        checkeq(access_scanner_skips,
                num_shards,
                "Expected access_scanner_skips to have reached num_shards");
    }
    ...
    std::map<std::string_view, std::vector<std::string_view> > statsKeys{
            ...
            {"config",
             {"ep_allow_sanitize_value_in_deletion",
              ...
              "ep_magma_fusion_endpoint_uri",
              "ep_magma_fusion_volume_name",
              "ep_magma_fusion_cache_size",
              ...
              "ep_warmup_secondary_min_items_threshold",
              "ep_warmup_secondary_min_memory_threshold",
              "ep_xattr_enabled"}},
            {"workload",
