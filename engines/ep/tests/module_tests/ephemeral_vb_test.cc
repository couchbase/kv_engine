...
TEST_F(EphemeralVBucketTest, DoublePageOut) {
    ...
    EXPECT_TRUE(
            vbucket->pageOut(rlh, readHandle, lock_sv.first, storedVal, false));
...
TEST_F(EphemeralVBucketTest, PageOutAfterCollectionsDrop) {
    ...
    // here.
    EXPECT_TRUE(
            vbucket->pageOut(rlh, readHandle, lock_sv.first, storedVal, true));
...
TEST_F(EphemeralVBucketTest, CreatePageoutCreate) {
    ...
    ASSERT_EQ(AddStatus::Success, addOne(key));
    {
        ...
        EXPECT_TRUE(vbucket->pageOut(
                rlh, readHandle, lock_sv.first, lock_sv.second, false));
        EXPECT_EQ(0, readHandle.getItemCount(key.getCollectionID()));
    }
...
TEST_F(EphemeralVBucketTest, CannotPageOutCommittedAndPendingKey) {
    ...
    ASSERT_FALSE(
            vbucket->isEligibleForEviction(result.lock, *result.storedValue));
}
