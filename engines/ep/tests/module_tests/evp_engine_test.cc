...
TEST_P(EPEngineParamTest, WorkloadPatternDefault) {
    ...
    EXPECT_EQ(cb::config::WorkloadPatternDefault::Mixed,
    ...
    EXPECT_EQ(cb::config::WorkloadPatternDefault::ReadHeavy,
              engine->getConfiguration().getWorkloadPatternDefault())
...
std::string createEncryptedFile(const std::filesystem::path file) {
    ...
    SharedEncryptionKey key = DataEncryptionKey::generate();
