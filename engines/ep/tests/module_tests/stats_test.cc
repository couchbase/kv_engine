...
TEST_P(ParameterizedStatTest, DiskSlownessSlowOp) {
    ...
    // Simulate read starting on a READER_TASK_IDX.
...
TEST_P(ParameterizedStatTest, DiskSlownessSlowOpNonReadWriteThread) {
    ...
    // Simulate read starting on a NONIO_TASK_IDX. Make sure we don't count
    // those.
...
protected:
    ...
    void setStatsToNonDefaultValues() {
        ...
        stats.replicaHTMemory.store(nonDefaultCounterValue);
        stats.replicaCheckpointOverhead.store(nonDefaultCounterValue);
    ...
    void checkStatsAreDefaultValues(bool initialization) {
        ...
        EXPECT_EQ(nonStatValue, stats.replicaHTMemory);
