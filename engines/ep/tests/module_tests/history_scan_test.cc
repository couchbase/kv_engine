...
TEST_P(HistoryScanTest, MB_56565) {
    ...
    // Now push the vbucket along - note the same key is to exploit CDC into
    // creating checkpoints and ensuring that DCP can't switch straight back
    // to in-memory
    ...
    // backfill:scan()
    //
    // Note with CDC - markDiskSnapshot occurs here if the backfill is 100%
    // history - otherwise markDiskSnapshot occurs in the create, making the
    // unit test harder to achieve (as we can't so easily interleave flush and
    // AUXIO), but also may make the bug less likely in a full stack.
    //
