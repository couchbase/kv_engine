...
TEST_P(EPBucketFullEvictionNoBloomFilterTest, CapTempItems) {
    ...
    for (int i = 0; i < 2; i++) {
        const auto key = "key-temp-init-" + std::to_string(i);
        const DocKey dockey(key, DocKeyEncodesCollectionId::No);
    ...
    key = "key-non-existent-4";
    const DocKey dockey4(key, DocKeyEncodesCollectionId::No);
    ...
    key = "key-non-existent-5";
    const DocKey dockey5(key, DocKeyEncodesCollectionId::No);
