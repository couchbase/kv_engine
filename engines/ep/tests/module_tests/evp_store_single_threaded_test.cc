...
void SingleThreadedKVBucketTest::shutdownAndPurgeTasks(EventuallyPersistentEngine* ep) {
    ...
    for (task_type_t t :
        ...
        task_executor->stopTaskGroup(ep->getTaskable().getGID(), t,
...
TEST_F(SingleThreadedEPBucketTest,
       MB29585_backfilling_whilst_snapshot_runs) {
    // Create a Mock Dcp producer and schedule on executorpool.
    mock_dcp_producer_t producer = new MockDcpProducer(*engine,
                                                       cookie,
                                                       "test_producer",
    ...
    // Create first stream
    auto vb = store->getVBucket(vbid);
    SingleThreadedRCPtr<MockActiveStream> stream = producer->mockActiveStreamRequest(/*flags*/ 0,
                                                   /*opaque*/ 0,
                                                   *vb,
                                                   /*st_seqno*/ 0,
                                                   /*en_seqno*/ ~0,
    ...
    EXPECT_STREQ("Process checkpoint(s) for DCP producer",
    ...
    runNextTask(lpAuxioQ, "Backfilling items for a DCP Connection");
       ...
       // After Backfilltask scheduled create(); should have received a disk
    // snapshot; which in turn calls markDiskShapshot to re-register cursor.
    EXPECT_EQ(2, vb->checkpointManager.getNumOfCursors()) << "Expected both persistence and "
                                               "replication cursors after "
                                               "markDiskShapshot";
    ...
    checkpointTask.runCurrentTask("Process checkpoint(s) for DCP producer");
