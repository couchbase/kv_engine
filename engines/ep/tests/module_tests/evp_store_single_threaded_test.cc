...
TEST_F(MB29369_SingleThreadedEPBucketTest,
    ...
    EXPECT_EQ(1, qNonIoQ.getFutureQueueSize())
            << "Expected to have ActiveStreamCheckpointProcessorTask in "
               "QuickNonIO Queue";
...
TEST_P(STParamPersistentBucketTest, MB29585_backfilling_whilst_snapshot_runs) {
    ...
    // with backfill later
    CheckedExecutor checkpointTask(task_executor, qNonIoQ);
...
TEST_P(STParamPersistentBucketTest,
    ...
    runNextTask(qNonIoQ, "Process checkpoint(s) for DCP producer " + testName);
    // BackfillManagerTask
    runNextTask(lpAuxioQ, "Backfilling items for MockDcpBackfillManager");
...
TEST_P(STParamPersistentBucketTest, MB19815_doDcpVbTakeoverStats) {
    ...
    EXPECT_NO_THROW(engine->public_doDcpVbTakeoverStats(
            *cookie, dummy_cb, "MB19815_doDCPVbTakeoverStats", vbid));
    destroy_mock_cookie(cookie);
...
TEST_P(MB20054_SingleThreadedEPStoreTest,
    ...
    EXPECT_EQ(0, qNonIoQ->getFutureQueueSize());
    EXPECT_EQ(0, qNonIoQ->getReadyQueueSize());
