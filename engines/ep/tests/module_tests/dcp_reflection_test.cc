...
// Test that dcp_cache_transfer_max_batch_items bounds how many items are
// encoded into a single DcpCacheTransfer message.
TEST_P(DCPCacheTransfer, CacheTransferMaxBatchItems) {
    // Limit batches to 2 items. The byte limit (default 2MiB) is not reached
    // by the small items of this test.
    engines[Node0]->getConfiguration().setDcpCacheTransferMaxBatchItems(2);
    ...
    // 4 items with a 2 item limit results in 2 CacheTransfer messages, each
    // carrying 2 items.
    ...
    for (int batch = 0; batch < 2; batch++) {
        ...
        EXPECT_EQ(2, static_cast<DcpCacheTransfer&>(*msg).getItems().size());
        route0_1.transferCache(*msg);
    }

    for (const auto& key : keys) {
        EXPECT_TRUE(replicaVB->ht.findForRead(key).storedValue);
    }
}
...
TEST_P(DCPCacheTransferVE, CacheTransferVE_ZeroResident) {
    auto keys = setupFBR(4);

    for (const auto& key : keys) {
        const char* msg;
        EXPECT_EQ(cb::engine_errc::success,
                  engines[Node0]->getKVBucket()->evictKey(key, vbid, &msg));
    }
    auto replicaVB = engines[Node1]->getKVBucket()->getVBucket(vbid);
    EXPECT_EQ(0, replicaVB->dirtyQueueSize);
    auto route0_1 = createDcpRoute(Node0, Node1);
    ...
    for (const auto& key : keys) {
        ASSERT_TRUE(replicaVB->ht.findForRead(key).storedValue);
        EXPECT_FALSE(replicaVB->ht.findForRead(key).storedValue->isDirty());
        EXPECT_FALSE(replicaVB->ht.findForRead(key).storedValue->isResident());
    }
    // Nothing is for flushing
    EXPECT_EQ(0, replicaVB->dirtyQueueSize);
}
...
INSTANTIATE_TEST_SUITE_P(DCPReflectionCacheTransferVE,
                         DCPCacheTransferVE,
