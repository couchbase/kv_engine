...
TEST_P(DurabilityActiveStreamTest, DiskSnapshotSendsHCSWithSyncRepSupport) {
    ...
    ASSERT_TRUE(isFlagSet(marker.getFlags(), DcpSnapshotMarkerFlag::Disk));
...
TEST_P(DurabilityPassiveStreamPersistentTest,
    ...
    {
        AbortSyncWriteConsumer msg(stream->getOpaque(),
                                   vbid,
    ...
    {
        MutationResponse msg(unrelated,
                             stream->getOpaque(),
                             IncludeDeleteTime::No,
                             DocKeyEncodesCollectionId::No,
                             EnableExpiryOutput::Yes,
...
TEST_P(DurabilityPassiveStreamTest, SeqnoAckAtSnapshotEndReceived) {
    ...
    // snapshot-end
    {
        auto msg = makeMutationResponse(snapEnd, vbid, value, opaque);
        ASSERT_EQ(cb::engine_errc::success, stream->messageReceived(msg));
    }
...
TEST_P(DurabilityPassiveStreamPersistentTest, DurabilityFence) {
    ...
    // s:5 non-durable -> no ack
    {
        auto msg = makeMutationResponse(5 /*seqno*/, vbid, value, opaque);
        ASSERT_EQ(cb::engine_errc::success, stream->messageReceived(msg));
    }
    EXPECT_EQ(0, readyQ.size());
    ...
    // s:9 non-durable -> no ack
    {
        auto msg = makeMutationResponse(9 /*seqno*/, vbid, value, opaque);
        ASSERT_EQ(cb::engine_errc::success, stream->messageReceived(msg));
    }
    EXPECT_EQ(0, readyQ.size());
    ...
    {
        auto msg = makeMutationResponse(10, vbid, value, opaque);
        ASSERT_EQ(cb::engine_errc::success, stream->messageReceived(msg));
    }
...
queued_item DurabilityPassiveStreamTest::makeAndReceiveSnapMarkerAndDcpPrepare(
        ...
        uint64_t seqno,
        cb::durability::Level level,
        DcpSnapshotMarkerFlag snapshotMarkerFlags,
...
void DurabilityPassiveStreamTest::testReceiveDuplicateDcpPrepare(
    ...
    // the DcpSnapshotMarkerFlag::Checkpoint set before the
    // Consumer receives the Commit. That will force the Consumer closing the
    // open checkpoint (which Contains the Prepare) and creating a new open one
    // for queueing the Commit.
    SnapshotMarker marker(
            opaque,
            vbid,
    ...
    {
        MutationResponse msg(std::move(qi),
                             opaque,
                             IncludeDeleteTime::No,
                             DocKeyEncodesCollectionId::No,
                             EnableExpiryOutput::Yes,
...
TEST_P(DurabilityPassiveStreamTest, ReceiveDuplicateDcpPrepareRemoveFromSet) {
    ...
    uint32_t opaque = 0;
    {
        MutationResponse msg(std::move(qi),
                             ...
                             EnableExpiryOutput::Yes,
                             cb::mcbp::DcpStreamId{});
        ASSERT_EQ(cb::engine_errc::out_of_range, stream->messageReceived(msg));
    }
}
...
TEST_P(DurabilityPassiveStreamPersistentTest,
    ...
    stream->processMarker(marker);
    {
        AbortSyncWriteConsumer msg(opaque, vbid, key, 1, 2);
...
void DurabilityPassiveStreamPersistentTest::
    ...
    const auto receiveSnapshot = [this, &vb, &ckptMgr](
        ...
        if (type == CheckpointType::Memory) {
            flags = DcpSnapshotMarkerFlag::Memory |
        ...
        if (type == CheckpointType::Memory) {
            CommitSyncWriteConsumer cmtMsg(opaque,
                                           vbid,
                                           snapStart /*prepareSeqno*/,
                                           snapEnd /*commitSeqno*/,
                                           key);
            EXPECT_EQ(cb::engine_errc::success,
                      stream->messageReceived(cmtMsg));
        } else {
            item = makeCommittedItem(key, value);
            item->setBySeqno(snapEnd);
            MutationResponse cmtMsg(std::move(item),
                                    opaque,
                                    IncludeDeleteTime::No,
                                    DocKeyEncodesCollectionId::No,
                                    EnableExpiryOutput::Yes,
...
void DurabilityPassiveStreamTest::testReceiveDcpAbort() {
    ...
    auto abortReceived = [this, opaque, &key](
        ...
        AbortSyncWriteConsumer msg(opaque, vbid, key, preSeqno, abortSeqno);
    ...
    // So, we need to simulate a Producer sending another SnapshotMarker with
    // the DcpSnapshotMarkerFlag::Checkpoint set before the
    // Consumer receives the Abort. That will force the Consumer closing the
    // open checkpoint (which Contains the Prepare) and cretaing a new open one
    // for queueing the Abort.
    SnapshotMarker marker(
            opaque,
            vbid,
            3 /*snapStart*/,
...
TEST_P(DurabilityPassiveStreamTest, ReceiveAbortWithoutPrepare) {
    ...
    auto abortSeqno = 2;
    {
        AbortSyncWriteConsumer msg(opaque, vbid, key, prepareSeqno, abortSeqno);
...
TEST_P(DurabilityPassiveStreamTest, CheckHCSUpdatedOnReplicaWarmup) {
    ...
    {
        auto msg = makeMutationResponse(1, vbid, "value", 0);
        stream->messageReceived(msg);
    }
    {
        auto msg = makeMutationResponse(2, vbid, "value", 0);
        stream->messageReceived(msg);
    }
    {
    ...
    {
        auto msg = makeMutationResponse(4, vbid, "value", 0);
        stream->messageReceived(msg);
    }
...
TEST_P(DurabilityPassiveStreamTest,
    ...
    {
        MutationResponse msg(committed,
                             opaque,
                             IncludeDeleteTime::No,
                             DocKeyEncodesCollectionId::No,
                             EnableExpiryOutput::Yes,
...
void DurabilityPassiveStreamTest::testPrepareCompletedAtAbort(
    ...
    {
        AbortSyncWriteConsumer msg(
                opaque, vbid, key, 10 /*prepareSeqno*/, 11 /*abortSeqno*/);
...
TEST_P(DurabilityPassiveStreamPersistentTest, FlowControlUnackedDcpCommit) {
    ...
    // No change, commit hasn't been acked
    EXPECT_EQ(ackBytes, consumer->getFlowControl().getFreedBytes());
 ...
 * and verifies that the DcpSnapshotMarkerFlag::Checkpoint is
 * set in all SnapshotMarkers sent to a Replica.
 */
void DurabilityPromotionStreamTest::
...

    {
        MutationResponse msg(std::move(item),
                             ...
                             cb::mcbp::DcpStreamId{});
        EXPECT_EQ(cb::engine_errc::success,
                  passiveStream->messageReceived(msg));
    }
    ...
    //   DcpSnapshotMarkerFlag::Checkpoint because we do not
    //   cover the case of (Memory->Disk snapshot transition). That means that a
    //   Replica will not create a new Checkpoint at receiving the marker and
    //   will just queue (or at least it will try to) the snapshot into the open
    //   Checkpoint, which may even be of different type.
    resp = activeStream->public_nextQueuedItem(*producer);
    ASSERT_TRUE(resp);
    ASSERT_EQ(DcpResponse::Event::SnapshotMarker, resp->getEvent());
    const auto& markerDisk = dynamic_cast<SnapshotMarker&>(*resp);
    ...
    //
    // !! NOTE: This is the important part of the test !!
    //   The marker must have the
    //   DcpSnapshotMarkerFlag::Checkpoint. Note that we
    //   covered this case (Disk -> Memory snapshot transition) even before this
    //   patch.
    resp = activeStream->public_nextQueuedItem(*producer);
    ASSERT_TRUE(resp);
    ASSERT_EQ(1, activeStream->public_readyQSize());
...
TEST_P(DurabilityPromotionStreamTest,
    ...
    mutation->setCas(1);
    {
        MutationResponse msg(mutation,
                             ...
                             cb::mcbp::DcpStreamId{});
        EXPECT_EQ(cb::engine_errc::success,
                  DurabilityPassiveStreamTest::stream->messageReceived(msg));
    }
    ...
    {
        AbortSyncWriteConsumer msg(
                0 /*opaque*/, vbid, key, 2 /*prepareSeqno*/, 3 /*abortSeqno*/);
        EXPECT_EQ(cb::engine_errc::success,
                  DurabilityPassiveStreamTest::stream->messageReceived(msg));
    }
...
TEST_P(DurabilityActiveStreamTest, inMemoryMultipleMarkers) {
    ...
    EXPECT_EQ(DcpResponse::Event::SnapshotMarker, resp->getEvent());
    auto snapMarker = dynamic_cast<SnapshotMarker&>(*resp);
    ASSERT_TRUE(isFlagSet(snapMarker.getFlags(),
                          DcpSnapshotMarkerFlag::Checkpoint));
    EXPECT_EQ(0, snapMarker.getStartSeqno());
    EXPECT_EQ(1, snapMarker.getEndSeqno());
...
void DurabilityPassiveStreamEphemeralTest::
    ...
    {
        AbortSyncWriteConsumer msg(0 /*opaque*/, vbid, key, 1, seqno++);
        stream->messageReceived(msg);
    }
    makeAndReceiveCommittedItem(key, cas, seqno, deleted);
