...
TEST_P(KVBucketParamTest, AddDeleted) {
    ...
    if (persistent()) {
        EXPECT_EQ(ENGINE_EWOULDBLOCK, store->add(deletedItem, cookie))
        ...
        runBGFetcherTask();
        EXPECT_EQ(ENGINE_SUCCESS, store->add(deletedItem, cookie))
    ...
    EXPECT_EQ(ENGINE_NOT_STORED, store->add(deletedItem, cookie))
    ...
    if (persistent()) {
        ...
        deleted2.setDeleted();
        EXPECT_EQ(ENGINE_EWOULDBLOCK, store->add(deleted2, cookie))
                << "Add() of deleted item (when tombstone ejected) should "
                   "require bgFetch";

        runBGFetcherTask();
        EXPECT_EQ(ENGINE_NOT_STORED, store->add(deleted2, cookie))
    ...
    ASSERT_EQ(ENGINE_SUCCESS, store->add(aliveItem, cookie));

    auto deleted4 = make_item(vbid, key, "deleted value 4");
    deleted4.setDeleted();
    EXPECT_EQ(ENGINE_NOT_STORED, store->add(deleted4, cookie))
