...
TEST_P(DurabilityBucketTest, RunCompletionTaskNoVBucket) {
    ...
    runNextTask(nonIoQ, "Notify clients of Sync Write Ambiguous vb:0");
    runNextTask(qNonIoQ, task->getDescription());
}
    ...
    ASSERT_EQ(cb::engine_errc::would_block,
              engine->store(*cookie,
                            *pending.get(),
                            ...
                            DocumentState::Alive,
                            false));
    ASSERT_TRUE(engine->getEngineSpecific<SyncWriteCookieState>(*cookie))
