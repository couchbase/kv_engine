...
HashTableTest::HashTableTest()
    : defaultHtSize(Configuration().getHtSize()),
...
TEST_F(HashTableTest, reallocateStoredValue) {
    ...
    HashTable ht1(global_stats,
                  makeFactory(true),
                  1,
                  1,
                  0,
                  defaultHtTempItemsAllowedPercent);
    HashTable ht2(global_stats,
                  makeFactory(true),
                  2,
                  2,
                  0,
                  defaultHtTempItemsAllowedPercent);
    HashTable ht3(global_stats,
                  makeFactory(true),
                  2,
                  2,
                  0,
...
TEST_F(HashTableTest, ReallocateStoredValue_locked) {
    ...
    StoredValue* v = ht.findForWrite(key, WantsDeleted::No).storedValue;
    ASSERT_TRUE(v);
    v->lock(100, 0xca5); // lock and set cas=0xca5
    ASSERT_EQ(0xca5, v->getCasForWrite(100));
    ASSERT_TRUE(v->isLocked(100));
    EXPECT_TRUE(ht.reallocateStoredValue(std::forward<StoredValue>(*v)));

    // Find it again, CAS for write must still be 0xca5
    v = ht.findForWrite(key, WantsDeleted::No).storedValue;
    ASSERT_TRUE(v->isLocked(100));
    ASSERT_EQ(0xca5, v->getCasForWrite(100))
...
public:
    GetRandomHashTable(EPStats& st,
                       ...
                       size_t tempItemsAllowedPercent)
        : HashTable(st,
                    std::move(svFactory),
                    initialSize,
                    locks,
...
public:
    GetRandomHashTable h{global_stats,
                         makeFactory(),
                         1,
                         1,
                         0,
