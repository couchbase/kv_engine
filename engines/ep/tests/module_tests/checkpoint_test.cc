...
TEST_P(CheckpointTest, ItemBasedCheckpointCreation) {
    ...
    config.setChkMaxItems(maxItems + 1);
    checkpoint_config = std::make_unique<CheckpointConfig>(config);
    createManager();
    ...
    for (unsigned int ii = 0; ii < maxItems; ii++) {
        EXPECT_EQ(ii + 1, this->manager->getNumOpenChkItems());
    ...
    EXPECT_EQ(2, this->manager->getNumCheckpoints());
    EXPECT_EQ(2, this->manager->getNumOpenChkItems()); // cs + 1x op_set

    // Fill up this checkpoint also - note loop for maxItems - 1
    for (unsigned int ii = 0; ii < maxItems - 1; ii++) {
        // +1 cs, +1 initial set
        EXPECT_EQ(ii + 1 + 1, this->manager->getNumOpenChkItems());
    ...
    EXPECT_EQ(2, this->manager->getNumCheckpoints());
    EXPECT_EQ(12, // cs, 1x key_epoch, 9x key_X, 1x key_epoch2
...
TEST_P(CheckpointTest, CursorOffsetOnCheckpointClose) {
    ...
    // cs, mut, mut, ce
    EXPECT_EQ(4, manager->removeClosedUnrefCheckpoints().count);
...
TEST_P(CheckpointTest, ItemsForCheckpointCursor) {
    ...
    /* Check if we have desired number of checkpoints and desired number of
       items */
    EXPECT_EQ(2, this->manager->getNumCheckpoints());
    EXPECT_EQ(maxItems + 1, this->manager->getNumOpenChkItems());
...
TEST_P(CheckpointTest, ItemsForCheckpointCursorLimited) {
    ...
    ASSERT_GT(manager->getNumOpenChkItems(), 0); // cs + some mutation
    ...
    EXPECT_EQ(1, result.ranges.size());
    EXPECT_EQ(0, result.ranges.front().getStart());
    EXPECT_GT(result.ranges.front().getEnd(), 1000);
    // Cursor at the begin of the second checkpoint, but second checkpoint not
    // processed
...
TEST_P(CheckpointTest, DiskCheckpointStrictItemLimit) {
    ...
    /* Verify we have desired number of checkpoints and desired number of
       items */
    ASSERT_EQ(2, this->manager->getNumCheckpoints());
    ASSERT_GT(manager->getNumOpenChkItems(), 1); // cs + some mutation
...
TEST_P(CheckpointTest, CursorMovement) {
    ...
    // Note: Consider that the checkpoint_start item is accounted at checkpoint
    // creation
    for (unsigned int ii = 0; ii < maxItems - 1; ii++) {
        EXPECT_TRUE(this->queueNewItem("key" + std::to_string(ii)));
    }
    ...
    /* Get items for persistence cursor, cs is available but not counted */
    EXPECT_EQ(0, manager->getNumItemsForCursor(cursor));
    items.clear();
    ...
    EXPECT_EQ(1, items.size());
    EXPECT_EQ(1, result.ranges.size());
    EXPECT_EQ(1000 + maxItems, items.front()->getBySeqno());
...
TEST_P(CheckpointTest, SeqnoAndHLCOrdering) {
    ...
    // data checks.
    // Note: Checkpoint numItems now accounts meta-items too, +1 to consider
    // the checkpoint_start
    config.setChkMaxItems(n_threads * n_items + 1);
    checkpoint_config = std::make_unique<CheckpointConfig>(config);
    // Also, we want to have items across 2 checkpoints.
    ASSERT_EQ(2, checkpoint_config->getMaxCheckpoints());
...
TEST_F(SingleThreadedCheckpointTest, CloseReplicaCheckpointOnDiskSnapshotEnd) {
    ...
    const size_t initialNumItems = 2;
    EXPECT_EQ(initialNumItems, ckptList.back()->getNumItems());
    EXPECT_EQ(initialNumItems, ckptMgr->getNumItems());
    ...
    EXPECT_EQ(ckptList.back()->getState(), checkpoint_state::CHECKPOINT_OPEN);
    EXPECT_EQ(ckptList.back()->getId(), 1);
    EXPECT_EQ(initialNumItems + (snapshotEnd - 1),
    ...
    EXPECT_EQ(ckptList.back()->getState(), checkpoint_state::CHECKPOINT_OPEN);
    EXPECT_EQ(ckptList.back()->getId(), 1);
    EXPECT_EQ(initialNumItems + snapshotEnd, ckptMgr->getNumOpenChkItems());
    ...
    EXPECT_EQ(1, ckptMgr->getNumOpenChkItems()); // cs
...
void SingleThreadedCheckpointTest::closeReplicaCheckpointOnMemorySnapshotEnd(
    ...
    EXPECT_EQ(ckptMgr->getNumOfCursors(), 1);
    // We must have only the checkpoint_start meta-items in the open checkpoint
    EXPECT_EQ(1, ckptMgr->getNumItems());
    ...
    // mem_used<high_wat, so no further checkpoint was created
    EXPECT_EQ(openedCheckPoints, ckptMgr->getNumCheckpoints());
...
void SingleThreadedCheckpointTest::testMinimumCursorSeqno(
    ...
    // Collateral check in this test: Verify that we always go to the process of
    // removing the previous instance of a cursor when we re-register it with
    // the same name. One of the effect of that is that the num-cursor
    // accounting in checkpoint is correctly handled.
    // Here we register the same test cursor that is then re-registered in the
    // following steps.
    // Then at the end of the test we verify that num-cursor accounting in
    // checkpoint hasn't changed.
    ASSERT_TRUE(manager.getPersistenceCursor());
    ASSERT_EQ(1, checkpoint.getNumCursorsInCheckpoint());
    const std::string testCursorName = "test-cursor";
    auto testCursor =
            manager.registerCursorBySeqno(
                           testCursorName, 0, CheckpointCursor::Droppable::Yes)
                    .cursor.lock();
    ASSERT_EQ(2, checkpoint.getNumCursorsInCheckpoint());
    ...
    case ItemRemovalPath::None: {
        ...
        const auto res = manager.registerCursorBySeqno(
                testCursorName, 1, CheckpointCursor::Droppable::Yes);
    ...
    case ItemRemovalPath::Expel: {
        ...
        // Move cursor to allow Expel
        flushVBucket(vbid);
        std::vector<queued_item> items;
        manager.getItemsForCursor(testCursor.get(), items, 123456789);
    ...
    // Num-cursors accounting in checkpoint hasn't changed.
    // If we missed to process cursor-removal, then we would account 3 cursor at
    // this point.
    EXPECT_EQ(2, checkpoint.getNumCursorsInCheckpoint());
}
...
TEST_P(CheckpointTest, TakeAndResetCursors) {
    ...
    for (const auto* c : cursors) {
        ASSERT_NE(nullptr, c);
        // cs is available but not counted
...
TEST_P(CheckpointTest, ExpelCheckpointItemsWithDuplicate) {
    ...
    // cs + mutation
    EXPECT_EQ(2, manager->getNumItemsForCursor(cursor2.cursor.lock().get()))
            << *manager;
...
TEST_P(EagerCheckpointDisposalTest, MB_58342) {
    ...
    EXPECT_EQ(0, manager->getNumItemsForCursor(cursor)) << *manager;
}
...
/**
 * MB-55520
 * This test ensures that, when in "eager" checkpoint removal mode, in a
 * sequence of [Memory, Disk] checkpoints the CM never returns checkpoints of
 * different types from CM::getItemsForCursor().
 *
 * Note that the issue that we fix/cover here affects only the "eager"
 * checkpoint removal behaviour (which is the default though in the Neo branch).
 *
 * We have already test coverage for the same in the "lazy" checkpoint removal
 * scenario. See CheckpointTest.getItems_MemoryDiskSnapshot.
 *
 * I write a dedicated test here for MB-55520 as this test highlights the
 * aspects relative to the particular failure seen in MB-55520.
 */
TEST_P(EagerCheckpointDisposalTest, NeverMergeCheckpointsOfDifferentType) {
    ...
    std::vector<queued_item> items;
    const auto res = manager->getItemsForCursor(cursor, items, 123456);
...
TEST_P(CheckpointTestDedup, dedupK1) {
    ...
    EXPECT_EQ(initialC2Items, manager->getNumItemsForCursor(c2.lock().get()));
    // k3 + k1
    EXPECT_EQ(initialC3Items + 1, manager->getNumItemsForCursor(c3.lock().get()));
}
