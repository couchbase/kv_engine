...
TEST_P(CheckpointTest, MB_47516) {
    ...
    // 3) Next we switch VB state (pending to active). Here we run the first CP
    //    function that occur in that path.
    //    The checkpoint ID is changed (see VBucket::setState_UNLOCKED). Here
    //    the queued set_vbucket_state Item gets changed from high-seqno+1 to
    //    low-seqno (1001)
    this->manager->setOpenCheckpointId(2);

    // 4) Expel occurs in the middle of the state switch - between
    //    setOpenCheckpointId and createNewCheckpoint. This is the second
