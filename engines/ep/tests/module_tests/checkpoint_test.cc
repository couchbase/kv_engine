...
CheckpointTest<V>::~CheckpointTest() {
    manager.reset();
    // Finally, check if the memory overhead is now back to the value at the
    // start.
    EXPECT_EQ(expectedOverhead, global_stats.getMemOverhead());
}

template <typename V>
void CheckpointTest<V>::createManager(int64_t last_seqno) {
    manager.reset();
    // Before constructing the manager, read the current mem-overhead, we should
    // get back to this value once a test completes
    expectedOverhead = global_stats.getMemOverhead();
...
template <typename V>
bool CheckpointTest<V>::queueReplicatedItem(const std::string& key,
                                            int64_t seqno) {
    queued_item qi{new Item(makeStoredDocKey(key),
                            this->vbucket->getId(),
                            queue_op::mutation,
...
TYPED_TEST(CheckpointTest, getItems_MultipleSnapshots) {
    ...
    std::vector<queued_item> items;
    auto cursorResult = this->manager->getItemsForPersistence(items, 1000);
...
TYPED_TEST(CheckpointTest, getItems_MemoryDiskSnapshots) {
    // 1st Snapshot covers 1001, 1003, but item 1002 de-duped
    this->manager->createSnapshot(1001, 1003, {}, CheckpointType::Memory);
    EXPECT_TRUE(this->queueReplicatedItem("k1", 1001));
    EXPECT_TRUE(this->queueReplicatedItem("k2", 1003));
...
TYPED_TEST(CheckpointTest, expelCheckpointItemsMemoryRecoveredTest) {
    ...
    // 2 = the number of our items that are expected to be expelled + 1
