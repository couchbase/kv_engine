    ...
    do {
        if (!disable_ack && (bytes_read > ack_limit)) {
            ...
            try {
                ...
                check_expression(false, "Aborting");
            }
        ...
        if (err == cb::engine_errc::disconnect) {
            ...
            default:
                ...
                check_expression(false, ss.str().c_str());
            }
    ...
    for (const auto& ctx : stream_ctxs) {
        if (!ctx.skip_verification) {
            ...
            if (!ctx.exp_collection_ids.empty()) {
                check_expression(
                        ctx.exp_collection_ids == stats.collections,
...
cb::engine_errc TestDcpConsumer::openStreams() {
    for (auto& ctx : stream_ctxs) {
        ...
        } else if (isFlagSet(ctx.flags, cb::mcbp::DcpAddStreamFlag::ToLatest) ||
    ...
    do {
        ...
        if (err == cb::engine_errc::disconnect) {
            ...
            default:
                ...
                check_expression(false, err_string.c_str());
            }
...
static enum test_result test_dcp_consumer_hotness_data(EngineIface* h) {
    ...
    check_expression(set_vbucket_state(h, vbid, vbucket_state_active),
                     "Failed to set vbucket state.");
...
static enum test_result test_dcp_producer_stream_latest(EngineIface* h) {
    ...
    DcpStreamCtx ctx;
    ctx.flags = cb::mcbp::DcpAddStreamFlag::ToLatest;
    ctx.vb_uuid = get_ull_stat(h, "vb_0:0:id", "failovers");
    ctx.seqno = {200, 205};
    ctx.snapshot = {200, 200};
    ctx.exp_mutations = 100;
...
static enum test_result test_dcp_producer_keep_stream_open(EngineIface* h) {
    ...
    wait_for_val_to_be(
            "last_sent_seqno", consumer->producers.last_byseqno, exp_items);

    // Check if the stream is still open after sending out latest items
    std::string stat_stream_state("eq_dcpq:" + conn_name + ":stream_0_state");
    std::string state = get_str_stat(h, stat_stream_state, "dcp");
    checkeq(state.compare("in-memory"), 0, "Stream is not open");

    // Before closing the connection stop the thread that continuously polls for
    // dcp data
    consumer->stop();
...
static test_result test_dcp_takeover(EngineIface* h) {
    ...
    DcpStreamCtx ctx;
    ctx.flags = cb::mcbp::DcpAddStreamFlag::TakeOver;
    ctx.vb_uuid = get_ull_stat(h, "vb_0:0:id", "failovers");
    ctx.seqno = {0, 1000};
    ...
    check_expression(verify_vbucket_state(h, Vbid(0), vbucket_state_dead),
                     "Wrong vb state");
...
static test_result test_dcp_takeover_no_items(EngineIface* h) {
    ...
    Vbid vbucket = Vbid(0);
    auto flags = cb::mcbp::DcpAddStreamFlag::TakeOver;
...
static enum test_result test_rollback_to_zero(EngineIface* h) {
    ...
    add_stream_for_consumer(
            h, cookie, opaque++, Vbid(0), {}, cb::mcbp::Status::Rollback);
...
static test_result test_chk_manager_rollback(EngineIface* engine) {
    ...
    checkeq(cb::engine_errc::success,
            dcp->add_stream(*cookie, ++opaque, vbid, {}),
            "Add stream request failed");
...
static test_result test_fullrollback_for_consumer(EngineIface* engine) {
    ...
    check_expression(set_vbucket_state(engine, Vbid(0), vbucket_state_replica),
                     "Failed to set vbucket state.");
...
static test_result test_dcp_add_stream_exists(EngineIface* h) {
    ...
    check_expression(set_vbucket_state(h, vbucket, vbucket_state_replica),
                     "Failed to set vbucket state.");
...
static enum test_result test_dcp_add_stream_prod_exists(EngineIface* h) {
    ...
    add_stream_for_consumer(
            h, cookie, opaque++, Vbid(0), {}, cb::mcbp::Status::KeyEexists);
    testHarness->destroy_cookie(cookie);
    return SUCCESS;
}
...
static enum test_result test_dcp_add_stream_prod_nmvb(EngineIface* h) {
    ...
    add_stream_for_consumer(
            h, cookie, opaque++, Vbid(0), {}, cb::mcbp::Status::NotMyVbucket);
    testHarness->destroy_cookie(cookie);
    return SUCCESS;
}
...
static enum test_result test_dcp_persistence_seqno_backfillItems(
    ...
    opaque = add_stream_for_consumer(
            h, consumerCookie, opaque, Vbid(0), {}, cb::mcbp::Status::Success);
...
static enum test_result test_failover_log_dcp(EngineIface* h) {
    ...
    dcp_params_t params[] = {
            ...
            {{}, uuid, 10, 0, high_seqno, 0, cb::engine_errc::success},
            {{}, uuid, 10, 0, high_seqno - 1, 0, cb::engine_errc::success},
            ...
            {{}, uuid, 20, 10, high_seqno + 1, 10, cb::engine_errc::rollback},
            ...
            {{}, 0xBAD, 10, 0, high_seqno, 0, cb::engine_errc::rollback},
...
static enum test_result test_dcp_on_vbucket_state_change(EngineIface* h) {
    ...
    // for stream closure
    check_expression(cb::mcbp::DcpStreamEndStatus::StateChanged ==
                             consumer->producers.last_end_status,
...
static enum test_result test_get_all_vb_seqnos(EngineIface* h) {
    ...
    check_expression(set_vbucket_state(h, rep_vb_num, vbucket_state_replica),
                     "Failed to set vbucket state");
    wait_for_flusher_to_settle(h);
  ...
  "scopes": [
    ...
    check_expression(
            set_vbucket_state(h, Vbid(num_vbuckets), vbucket_state_pending),
            "Failed to set vbucket state.");
...
static enum test_result test_MB_34634(EngineIface* h) {
    ...
    check_expression(set_vbucket_state(h, vb, vbucket_state_pending),
    ...
    opaque = add_stream_for_consumer(
            h, cookie, opaque, vb, {}, cb::mcbp::Status::Success);
    ...
    checkeq(cb::engine_errc::success,
            dcp->add_stream(
                    *cookie, opaque, vb, cb::mcbp::DcpAddStreamFlag::TakeOver),
            "Add stream request failed");
