...
// Similary for format.h which defines a 'cb_check()' function:
...
static void dcpHandleResponse(EngineIface* h,
    ...
    check_expression(erroCode == cb::engine_errc::success ||
    ...
    for (const auto& ctx : stream_ctxs) {
        if (!ctx.skip_verification) {
            ...
            if (!ctx.exp_collection_ids.empty()) {
                check_expression(ctx.exp_collection_ids == stats.collections,
...
static enum test_result test_dcp_producer_keep_stream_open_replica(
    ...
    std::string stat_stream_last_sent_snap_end_seqno(
            "eq_dcpq:" + conn_name1 + ":stream_" + std::to_string(vb) +
...
static enum test_result test_get_all_vb_seqnos(EngineIface* h) {
  ...
  "scopes": [
    ...
    check_expression(set_vbucket_state(h, Vbid(num_vbuckets), vbucket_state_pending),
          "Failed to set vbucket state.");
