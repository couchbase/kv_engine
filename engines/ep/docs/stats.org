...
| ep_warmup_accesslog_load_batch_size   | The maximum #keys loaded in a single    |
|                                       | during the access log warmup phases.    |
|                                       | warmup                                  |
| ep_warmup_dups                        | Number of Duplicate items encountered   |
|                                       | during warmup                           |
| ep_primary_warmup_min_items_threshold | Percentage of total items warmed up     |
|                                       | before we enable traffic                |
| ep_primary_warmup_min_memory_threshold| Percentage of max mem warmed up before  |
|                                       | we enable traffic                       |
...
| curr_items                                              | Number of active non-deleted items             |
...
| vb_active_sync_write_committed_not_durable_count | Number of SyncWrites committed without replica ack (fallback) |
...
| vb_replica_sync_write_committed_not_durable_count | Number of SyncWrites committed without replica ack (fallback) |
...
| Stat                          | Description                                |
|-------------------------------+--------------------------------------------|
| num_items                     | Number of items in this vbucket (excludes deleted, prepares and system items) |
...
| item_alloc_sizes                | Item allocation size counters for front-end    |
|                                 | mutations (in bytes)                           |
...
** Disk Slowness Stats
The `disk-slowness <threshold:seconds>` group provides an information about the
pending disk requests as well as the currently longest-running request (the
pending request that has been blocked the longest). Note that only the reader
and writer threads are considered.

The threshold argument is required and controls the slowness threshold used for
determining the values of stats containing "_slow_".

| pending_disk_read_num       | current number of pending disk read requests                      |
| pending_disk_read_slow_num  | current number of pending disk read requests above the threshold  |
| pending_disk_write_num      | current number of pending disk write requests                     |
| pending_disk_write_slow_num | current number of pending disk write requests above the threshold |

For pending disk requests that are larger than the configured maximum document
size, we scale the threshold by the multiple of the document size.
...
* encryption-key-ids
See description in utilities/stat_group.cc
