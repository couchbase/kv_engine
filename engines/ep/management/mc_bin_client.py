...
class ErrorKeyEnoent(MemcachedError):
    ERRCODE = 0x1


class ErrorKeyEexists(MemcachedError):
    ERRCODE = 0x2


class ErrorE2big(MemcachedError):
    ERRCODE = 0x3


class ErrorEinval(MemcachedError):
    ERRCODE = 0x4


class ErrorNotStored(MemcachedError):
    ERRCODE = 0x5


class ErrorDeltaBadval(MemcachedError):
    ERRCODE = 0x6


class ErrorNotMyVbucket(MemcachedError):
    ERRCODE = 0x7


class ErrorNoBucket(MemcachedError):
    ERRCODE = 0x8


class ErrorLocked(MemcachedError):
    ERRCODE = 0x9


class ErrorAuthStale(MemcachedError):
    ERRCODE = 0x1f


class ErrorAuthError(MemcachedError):
    ERRCODE = 0x20


class ErrorAuthContinue(MemcachedError):
    ERRCODE = 0x21


class ErrorErange(MemcachedError):
    ERRCODE = 0x22


class ErrorRollback(MemcachedError):
    ERRCODE = 0x23


class ErrorEaccess(MemcachedError):
    ERRCODE = 0x24


class ErrorNotInitialized(MemcachedError):
    ERRCODE = 0x25


class ErrorUnknownCommand(MemcachedError):
    ERRCODE = 0x81


class ErrorEnomem(MemcachedError):
    ERRCODE = 0x82


class ErrorNotSupported(MemcachedError):
    ERRCODE = 0x83


class ErrorEinternal(MemcachedError):
    ERRCODE = 0x84


class ErrorEbusy(MemcachedError):
    ERRCODE = 0x85


class ErrorEtmpfail(MemcachedError):
    ERRCODE = 0x86


class ErrorXattrEinval(MemcachedError):
    ERRCODE = 0x87


class ErrorUnknownCollection(MemcachedError):
    ERRCODE = 0x88


class ErrorNoCollectionManifest(MemcachedError):
    ERRCODE = 0x89


class ErrorDurabilityInvalidLevel(MemcachedError):
    ERRCODE = 0xa0


class ErrorDurabilityImpossible(MemcachedError):
    ERRCODE = 0xa1


class ErrorSyncWriteInProgress(MemcachedError):
    ERRCODE = 0xa2


class ErrorSyncWriteAmbiguous(MemcachedError):
    ERRCODE = 0xa3


class ErrorSyncWriteReCommitInProgress(MemcachedError):
    ERRCODE = 0xa4


class ErrorSubdocPathEnoent(MemcachedError):
    ERRCODE = 0xc0


class ErrorSubdocPathMismatch(MemcachedError):
    ERRCODE = 0xc1


class ErrorSubdocPathEinval(MemcachedError):
    ERRCODE = 0xc2


class ErrorSubdocPathE2big(MemcachedError):
    ERRCODE = 0xc3


class ErrorSubdocPathE2deep(MemcachedError):
    ERRCODE = 0xc4


class ErrorSubdocValueCantinsert(MemcachedError):
    ERRCODE = 0xc5


class ErrorSubdocDocNotjson(MemcachedError):
    ERRCODE = 0xc6


class ErrorSubdocNumErange(MemcachedError):
    ERRCODE = 0xc7


class ErrorSubdocDeltaEinval(MemcachedError):
    ERRCODE = 0xc8


class ErrorSubdocPathEexists(MemcachedError):
    ERRCODE = 0xc9


class ErrorSubdocValueEtoodeep(MemcachedError):
    ERRCODE = 0xca


class ErrorSubdocInvalidCombo(MemcachedError):
    ERRCODE = 0xcb


class ErrorSubdocMultiPathFailure(MemcachedError):
    ERRCODE = 0xcc


class ErrorSubdocSuccessDeleted(MemcachedError):
    ERRCODE = 0xcd


class ErrorSubdocXattrInvalidFlagCombo(MemcachedError):
    ERRCODE = 0xce


class ErrorSubdocXattrInvalidKeyCombo(MemcachedError):
    ERRCODE = 0xcf


class ErrorSubdocXattrUnknownMacro(MemcachedError):
    ERRCODE = 0xd0
...
class MemcachedClient(object):
    ...
    def _recvMsg(self):
        ...
        assert magic in (
            RES_MAGIC_BYTE, ALT_RES_MAGIC_BYTE), "Got magic: {:#x}".format(magic)
    ...
    def _mutateDurable(
        ...
        return self._doAltCmd(
            cmd, flex, key, val, struct.pack(
                SET_PKT_FMT, flags, exp), cas, collection)
    ...
    def append(self, key, value, cas=0, collection=None):
        return self._cat(memcacheConstants.CMD_APPEND,
                         key, cas, value, collection)

    def prepend(self, key, value, cas=0, collection=None):
        return self._cat(memcacheConstants.CMD_PREPEND,
                         key, cas, value, collection)

    def __incrdecr(self, cmd, key, amt, init, exp, collection):
        something, cas, val = self._doCmd(cmd, key, '', struct.pack(
            memcacheConstants.INCRDECR_PKT_FMT, amt, init, exp), collection=collection)
    ...
    def incr(self, key, amt=1, init=0, exp=0, collection=None):
        ...
        return self.__incrdecr(
            memcacheConstants.CMD_INCR,
            key,
            amt,
            init,
            exp,
    ...
    def set(self, key, exp, flags, val, collection=None):
        ...
        return self._mutate(memcacheConstants.CMD_SET, key,
                            exp, flags, 0, val, collection)
    ...
    def setWithMeta(
        ...
        * `SKIP_CONFLICT_RESOLUTION_FLAG`
        ...
        return self._doMetaCmd(
            memcacheConstants.CMD_SET_WITH_META,
            key,
            value,
            0,
            exp,
    ...
    def add(self, key, exp, flags, val, collection=None):
        ...
        return self._mutate(memcacheConstants.CMD_ADD, key,
                            exp, flags, 0, val, collection)
    ...
    def addWithMeta(
            ...
            collection=None):
        return self._doMetaCmd(
            memcacheConstants.CMD_ADD_WITH_META,
            key,
            value,
            0,
            exp,
    ...
    def replaceDurable(self, key, exp, flags, val,
        ...
        return self._mutateDurable(
            memcacheConstants.CMD_REPLACE,
            key,
            exp,
            flags,
            0,
    ...
    def observe(self, key, vbucket, collection=None):
        ...
        opaque, cas, data = self._doCmd(
            memcacheConstants.CMD_OBSERVE, '', value, collection=collection)
    ...
    def getMeta(self, key, collection=None):
        ...
        opaque, cas, data = self._doCmd(
            memcacheConstants.CMD_GET_META, key, '', collection=collection)
    ...
    def getl(self, key, exp=15, collection=None):
        """Get the value for a given key within the memcached server."""
        parts = self._doCmd(
            memcacheConstants.CMD_GET_LOCKED,
            key,
            '',
            struct.pack(
                memcacheConstants.GETL_PKT_FMT,
                exp),
            collection=collection)
        return self.__parseGet(parts)
    ...
    def touch(self, key, exp, collection=None):
        ...
        return self._doCmd(memcacheConstants.CMD_TOUCH, key, '', struct.pack(
            memcacheConstants.TOUCH_PKT_FMT, exp), collection=collection)

    def gat(self, key, exp, collection=None):
        """Get the value for a given key and touch it within the memcached server."""
        parts = self._doCmd(memcacheConstants.CMD_GAT, key, '', struct.pack(
            memcacheConstants.GAT_PKT_FMT, exp), collection=collection)
        return self.__parseGet(parts)

    def getr(self, key, collection=None):
        """Get the value for a given key in a replica vbucket within the memcached server."""
        parts = self._doCmd(
            memcacheConstants.CMD_GET_REPLICA,
            key,
            '',
            collection=collection)
        return self.__parseGet(parts)
    ...
    def sasl_auth_plain(self, user, password, foruser=''):
        """Perform plain auth."""
        return self.sasl_auth_start(
            'PLAIN', '\0'.join([foruser, user, password]))
    ...
    def evict_key(self, key, collection=None):
        return self._doCmd(memcacheConstants.CMD_EVICT_KEY,
    ...
    def getMulti(self, keys, collection=None):
        ...
        for k, v in opaqued.items():
            self._sendCmd(memcacheConstants.CMD_GETQ, v,
    ...
    def delMulti(self, items, collection=None):
        ...
        for opaque, k in opaqued.items():
            self._sendCmd(memcacheConstants.CMD_DELETEQ, k, '',
                          opaque, extra, collection=collection)
    ...
    def stats(self, sub='', val=''):
        ...
        rv = {}
        while not done:
            cmd, opaque, cas, klen, extralen, data = self._handleKeyedResponse(
                None)
            if klen:
    ...
    def flush(self, timebomb=0):
        ...
        return self._doCmd(
            memcacheConstants.CMD_FLUSH, '', '', struct.pack(
                memcacheConstants.FLUSH_PKT_FMT, timebomb))
    ...
    def _encodeCollectionId(self, key, collection):
        ...
        if isinstance(collection, str):
            ...
            try:
                ...
                print(
                    "Error: cannot map collection \"{}\" to an ID".format(collection))
