...
cbbackup incremental - incrementally backup couchbase

=head1 SYNOPSIS

=over 2

B<cbbackup incremental> [-h host:port] [-o output_filename] [-T timeout_secs] [-r] [-t transaction_size] [-v] tap_name

B<cbbackup incremental> db_file

=back

=head1 DESCRIPTION

B<cbbackup-incremental> gives one the ability to create an incremental backup
from a checkpoint created and managed by the Couchbase server.  In
conjunction with the output from B<cbbackup-incremental>, a Couchbase
server may be restored to a given consistent checkpoint.

B<cbbackup-incremental> can also verify a given backup file.
...
B<cbbackup incremental> via cron or some other
...
backups, to leave the most recent one.  This way, B<cbbackup>
...
my_backup_00000.cbb if it is the first file.  Otherwise
...
# cbbackup incremental -h myserver.bigco.com:11210 -o got_my_back%.cbb hourly_backup
