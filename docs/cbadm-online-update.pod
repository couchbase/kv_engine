...
B<cbadm online-update> host:port start|complete|revert

=back

=head1 DESCRIPTION

B<cbadm online-update> allows for a Couchbase cluster to enter a
...
by normal Couchbase operations such as get or TAP, the online-update
...
Note that the B<cbadm online-update start> command should only
...
clusters, though it may be compatible.  B<cbadm online-update> is not
