...
|                               | included in the avg                       |
| ep_tap_bg_min_wait            | The shortest time (ns) for a tap item     |
|                               | before it is serviced by the dispatcher   |
| ep_tap_bg_max_wait            | The longest time (ns) for a tap item      |
|                               | before it is serviced by the dispatcher   |
| ep_tap_bg_wait_avg            | The average wait time (ns) for a tap item |
|                               | before it is serviced by the dispatcher   |
| ep_tap_bg_min_load            | The shortest time (ns) for a tap item to  |
|                               | be loaded from the persistence layer      |
| ep_tap_bg_max_load            | The longest time (ns) for a tap item to   |
|                               | be loaded from the persistence layer      |
| ep_tap_bg_load_avg            | The average time (ns) for a tap item to   |
|                               | be loaded from the persistence layer      |
...
| ep_pending_ops_max_duration   | Max time (ns) used waiting on pending     |
|                               | vbuckets                                  |
| ep_bg_num_samples             | The number of samples included in the avg |
| ep_bg_min_wait                | The shortest time (ns) in the wait queue  |
| ep_bg_max_wait                | The longest time (ns) in the wait queue   |
| ep_bg_wait_avg                | The average wait time (ns) for an item    |
|                               | before it is serviced by the dispatcher   |
| ep_bg_min_load                | The shortest load time (ns)               |
| ep_bg_max_load                | The longest load time (ns)                |
| ep_bg_load_avg                | The average time (ns) for an item to be   |
